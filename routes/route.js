...
router.get("/success", (req, res) => {
  ...
  const checkstring = req.query.checkstring; // Add this line to get the checkstring

  res.render("success", {checkstring });
});

function generatepnr(length) {
    ...
    if (err) {
      ...
      try {
        ...
        req.session.message = {
          ...
          res.redirect(`/success?pnr=${checkstring}`);
      } catch (error) {
        req.session.message = {
          type: "danger",
...
router.get("/index", authenticateMiddleware, (req, res) => {
  res.render("index");
});
